...
MODEL_PATH = r"model\best_model_fd004.pth"
SCALER_PATH = r"scaler\scaler_fd004.pkl"
...
# Helper Functions
def load_model_and_scaler(fd_model="FD004"):
...
def build_last_windows(df):
    ...
    for uid in sorted(df["unit"].unique()):
        u_df = df[df["unit"] == uid]
        feats = u_df[SELECTED_FEATURES].values
        ...
        X.append(window)
        units.append(uid)
    return np.array(X, dtype=np.float32), units
...
def upload_and_run():
    ...
    fd_model = request.form.get('fd_model', 'FD004')
    ...
    try:
        ...
        true_vals = np.minimum(true_vals, RUL_CAP)  # ✅ REQUIRED FIX
    except Exception as e:
